...
//		rb2.setUserData("Calendar");	//선택했을때의 값을 나타내기 위한 데이터 설정
		
		RadioButton rb3 =new RadioButton("CONTACTS");
		rb3.setToggleGroup(group); //라디오 버튼을 그룹에 추가
//		rb3.setUserData("Contacts");	//선택했을때의 값을 나타내기 위한 데이터 설정
		...
		group.selectedToggleProperty().addListener(			
				new ChangeListener<Toggle>() {
					...
					public void changed(ObservableValue<? extends Toggle> observable, Toggle oldValue,
							Toggle newValue) {
						if(group.getSelectedToggle().getUserData() != null) {	//선택되어있는데 toggle들을 가져온다.
