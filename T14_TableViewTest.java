...
public class T14_TableViewTest extends Application {
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ObservableList<Member> data = FXCollections.observableArrayList(
					...
					new Member("홍길남", "gildnam", 44, "4444-4444", "부산"),
		...
		TableColumn<Member, String> telCol = new TableColumn<>("전화번호");	//컬럼 타이틀 세팅.
		
		telCol.setCellValueFactory(new PropertyValueFactory<>("tel"));		//세팅된Propertyvalue에서  tel를 가져온다. 
		...
		table.getColumns().addAll(nameCol, ageCol, telCol, addrCol);		//getColumns의 리턴타입이 ObserableList를 불러온다.
		...
		btnAdd.setOnAction(e->{
			...
			if(!Pattern.matches("^[0-9]+$", txtAge.getText())) {//숫자만 허용
//				System.out.println("데이터 오류");
				errMsg("데이터 오류", "나이는 정수형으로 입력하세요" );
				txtAge.requestFocus(); //해당 객체에 포커스 주기		//잘못 입력시 해당 항목에 포커스가 표시되게하는 requestFocus
				return;
				
			}
			data.add(new Member(txtKorName.getText(), txtEngName.getText(),
					Integer.parseInt(txtAge.getText()), txtTel.getText(),
					txtAddr.getText()));
//			System.out.println("정보 추가 성공...");
			infoMsg("작업 결과", txtKorName.getText()+"님 정보를 추가 했습니다.");
...
//				System.out.println("데이터 오류");
			...
			}	//table.getSelectionModel().getSelectedIndex() ObservableList에서 하는 해당 인덱스 값 가져온다. 
			data.set(table.getSelectionModel().getSelectedIndex(), new Member(txtKorName.getText(), txtEngName.getText(),	
					Integer.parseInt(txtAge.getText()), txtTel.getText(),
					txtAddr.getText()));
//			System.out.println("정보 추가 성공...");
			infoMsg("작업 결과", txtKorName.getText()+"님 정보를 수정 했습니다.");
			txtKorName.clear();
			txtEngName.clear();
			txtAge.clear();
			txtTel.clear();
